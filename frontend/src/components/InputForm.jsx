...
function matchOption(options, value, fallback = "") {
   ...
   const matched = options.find((item) => normalizeCompare(item) === normalized);
   return matched || fallback;
}

function normalizeToneSelection(rawValue) {
   const values = Array.isArray(rawValue) ? rawValue : rawValue ? [rawValue] : [];
   return values
      .map((value) => matchOption(TONES, value))
      .filter(Boolean);
}

function readingLevelToIndex(rawValue) {
   if (typeof rawValue === "number" && Number.isFinite(rawValue)) {
      return clamp(rawValue, 0, READING_LEVELS.length - 1);
   }
   const idx = READING_LEVELS.findIndex((item) => normalizeCompare(item) === normalizeCompare(rawValue));
   return idx >= 0 ? idx : 1;
}

function pacingToStep(rawValue) {
   if (typeof rawValue === "number" && Number.isFinite(rawValue)) {
      return clamp(rawValue, 1, PACING_LABELS.length);
   }
   const idx = PACING_LABELS.findIndex((item) => normalizeCompare(item) === normalizeCompare(rawValue));
   return idx >= 0 ? idx + 1 : 3;
}

function buildFormStateFromContext(context = {}) {
   return {
      genre: matchOption(GENRES, context?.writing?.genre, ""),
      subGenre: context?.writing?.sub_genre || "",
      writingPurpose: matchOption(PURPOSES, context?.writing?.purpose, "Entertain"),
      targetWordCount: context?.writing?.target_word_count || "Short (1k-10k)",
      draftStage: matchOption(DRAFT_STAGES, context?.writing?.draft_stage, "First Draft"),
      targetAgeGroup: matchOption(AGE_GROUPS, context?.audience?.age_group, "Adult"),
      readingLevel: readingLevelToIndex(context?.audience?.reading_level),
      audienceFamiliarity: matchOption(FAMILIARITY, context?.audience?.familiarity, "Genre fan"),
      marketPlatform: matchOption(PLATFORMS, context?.audience?.market_platform, "Self-publish"),
      desiredTone: normalizeToneSelection(context?.tone?.desired_tone),
      narrativeVoice: matchOption(VOICES, context?.tone?.narrative_voice, "Third limited"),
      pacingPreference: pacingToStep(context?.tone?.pacing_preference),
      contentSensitivity: matchOption(SENSITIVITY, context?.tone?.content_sensitivity, "PG-13"),
   };
}

export default function InputForm({ context, setContext, onComplete, onSkip = null }) {
   const [step, setStep] = useState(0);
   const [slideDir, setSlideDir] = useState("right");

   const [form, setForm] = useState(() => buildFormStateFromContext(context));

   useEffect(() => {
      setForm(buildFormStateFromContext(context));
   }, [context]);
      ...
      <div className="input-form-container">
         ...
         <div className="form-nav">
            ...
            {typeof onSkip === "function" ? (
               <button className="btn-ghost form-skip-btn" onClick={onSkip}>Skip Setup</button>
            ) : null}
            <div style={{ flex: 1 }} />
            {step < 3 ? (
