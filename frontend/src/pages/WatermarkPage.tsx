    ...
    return (
        <div className="p-6 lg:p-12 max-w-7xl mx-auto space-y-8">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="mb-6 bg-background border border-border shadow-sm">
                    <TabsTrigger value="embed" className="font-semibold uppercase text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                        <Lock className="h-4 w-4 mr-2" />Embed
                    </TabsTrigger>
                    <TabsTrigger value="detect" className="font-semibold uppercase text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                        <Eye className="h-4 w-4 mr-2" />Detect
                    </TabsTrigger>
                    <TabsTrigger value="signature" className="font-semibold uppercase text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-4">
                                ...
                                <Button onClick={handleEmbed} disabled={isProcessing}
                                    className="bg-primary text-primary-foreground px-6 font-semibold uppercase border border-border shadow-sm hover:translate-y-0.5 hover:shadow-sm transition-all rounded-md">
                        ...
                        {embedResult && (
                            <div className="space-y-4">
                                <div className="bg-card border border-border p-6 shadow-sm">
                                    <h3 className="font-semibold uppercase text-sm tracking-tight mb-4">Watermark Result</h3>
                                    <div className="space-y-3">
                                        <div className="flex justify-between text-sm">
                                            <span className="font-bold">Watermark ID</span>
                                            <span className="text-sm text-primary">{embedResult.watermark_id}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="font-bold">Bits Encoded</span>
                                            <span className="text-sm">{embedResult.bits_encoded} / {embedResult.bits_total}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="font-bold">Modifications</span>
                                            <span className="text-sm">{embedResult.modifications_made}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="font-bold">Success Rate</span>
                                            <span className="text-sm text-green-500">{embedResult.encoding_success_rate}%</span>
                                        </div>
                                        <Progress value={embedResult.encoding_success_rate} className="h-3" />
                                    </div>
                                </div>
                                <div className="bg-muted p-4 border border-border/40 max-h-48 overflow-auto scrollbar-thin">
                                    <p className="text-xs whitespace-pre-wrap">{embedResult.watermarked_text}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <Button onClick={handleDetect} disabled={isProcessing}
                                className="bg-foreground text-background px-8 font-semibold uppercase shadow-sm hover:translate-y-0.5 hover:shadow-sm transition-all rounded-md">
                        ...
                        {detectResult && (
                            <div className="bg-card border border-border p-6 shadow-sm">
                                <div className={`mb-4 flex items-center gap-3 rounded-md p-4 ${detectResult.watermark_detected ? 'border border-green-500 bg-green-500/10' : 'border border-red-500 bg-red-500/10'}`}>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="text-sm">{detectResult.confidence}%</span>
                                    </div>
                                    <Progress value={detectResult.confidence} className="h-3" />
                                    <div className="flex justify-between text-sm">
                                        <span className="font-bold">Bits Detected</span>
                                        <span className="text-sm">{detectResult.bits_detected}</span>
                                    </div>
                                    {detectResult.decoded_id && (
                                        <div className="flex justify-between text-sm">
                                            <span className="font-bold">Decoded ID</span>
                                            <span className="text-sm text-primary">{detectResult.decoded_id}</span>
                                        </div>
                                    )}
                                </div>
                                {detectResult.evidence.length > 0 && (
                                    <div className="mt-4">
                                        <h4 className="text-xs font-semibold uppercase mb-2">Evidence</h4>
                ...
                <TabsContent value="signature">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        {signature && (
                            <div className="bg-card border border-border p-6 shadow-sm">
                                <h3 className="font-semibold uppercase text-sm tracking-tight mb-4">Stylometric Signature</h3>
                                <div className="mb-4 break-all rounded-md border border-primary/40 bg-primary/10 p-3 text-sm">
                                    {signature.signature_hash}
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-muted">
                                        <p className="text-[10px] font-bold uppercase text-muted-foreground">Avg Word Length</p>
                                        <p className="text-xl font-semibold">{signature.avg_word_length}</p>
                                    </div>
                                    <div className="p-3 bg-muted">
                                        <p className="text-[10px] font-bold uppercase text-muted-foreground">Avg Sentence Length</p>
                                        <p className="text-xl font-semibold">{signature.avg_sentence_length}</p>
                                    </div>
                                    <div className="p-3 bg-muted">
                                        <p className="text-[10px] font-bold uppercase text-muted-foreground">Vocab Richness</p>
                                        <p className="text-xl font-semibold">{(signature.vocabulary_richness * 100).toFixed(1)}%</p>
                                    </div>
                                    <div className="p-3 bg-muted">
                                        <p className="text-[10px] font-bold uppercase text-muted-foreground">Hapax Ratio</p>
                                        <p className="text-xl font-semibold">{(signature.hapax_legomena_ratio * 100).toFixed(1)}%</p>
                                    </div>
                                </div>
                            </div>
                        )}
